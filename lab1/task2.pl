...
print_studens_summary :-
    ...
    forall(member(S, Students),
        (
            student_average_grade(S, Avg),
            (student_all_exams_passed(S) -> Status = 'All exams passed.' ; Status = 'Failed at least one exam.'),
...
print_subject_fail_summary :-
    ...
    forall(member(Subj, UniqueSubjects),
        (
            subject_failed_count(Subj, Count),
...
print_max_avgerage_in_group(Group) :-
    ...
    max_member(MaxAvg-_, AvgPairs),
    findall(Student, member(MaxAvg-Student, AvgPairs), TopStudents),
